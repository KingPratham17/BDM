...
export default function DocumentGenerator() {
  ...
  const resetTemplateForm = () => {
    ...
    setDocumentName('');
    // optional: reset any select visually if needed
    const selectEl = document.querySelector('.template-select-dropdown');
  ...
  // ====== AI clause generation (quick flow) ======
  const handleAIGenerateClauses = async (e) => {
    ...
    try {
      ...
      generatedClauses.forEach((clause) => {
        const txt = clause?.content || '';
        const matches = txt.match(/\[([^\]]+)\]/g);
        if (matches) {
          matches.forEach((m) => {
            const p = m.substring(1, m.length - 1).trim();
  ...
  const handleTranslateRequest = async (docId, lang) => {
    ...
    try {
      ...
      // fetch english assembled content (robust handling)
      const enResp = await documentsAPI.getContent(docId, 'en');
      let enText = '';
      if (enResp?.data) {
        if (typeof enResp.data === 'string') enText = enResp.data;
        else if (enResp.data.text) enText = enResp.data.text;
        else if (enResp.data.data && enResp.data.data.text) enText = enResp.data.data.text;
      }
  ...
  const handleTranslateConfirm = async () => {
    if (!translatePreview?.previewId) return showNotification('No preview to confirm', 'error');
  ...
  const handleTranslateDownloadPdf = async (which) => {
    ...
    if (translationIdSaved) body.translationId = translationIdSaved;
  ...
  const handlePreviewWithValues = () => {
    if (!selectedTemplate || !Array.isArray(selectedTemplate.clauses)) {
      return showNotification('Cannot preview: Template missing', 'error');
    }
  ...
  const handleDeleteDocument = async (docId) => {
    ...
    try {
      ...
      if (pdfUrl && pdfUrl.includes(`/documents/${docId}/`)) setPdfUrl(null);
  ...
  // ====== editor ======
  const handleEditDocument = (doc) => setEditingDocument(doc);
  ...
  // render input for placeholders (date/signature heuristics)
  const renderPlaceholderInput = (placeholder, isAiForm = false) => {
    ...
    const onChange = (e) => handlePlaceholderChange(placeholder, e.target.value, isAiForm);
    const id = `${placeholder.replace(/\s+/g, '-')}-${isAiForm ? 'ai' : 'template'}`;

    const dateKeywords = ['date', 'start date', 'end date', 'effective date', 'signing date'];
    const isDate = dateKeywords.includes(lower) || lower.endsWith(' date');

    const signatureKeywords = ['signature', 'candidate signature', 'employer signature'];
    const isSig = signatureKeywords.includes(lower) || lower.endsWith(' signature');

    if (isDate) return <input type="date" id={id} className="form-input" value={value || ''} onChange={onChange} required />;
    if (isSig)
      return <input type="text" id={id} className="form-input signature-input" value={value || ''} onChange={onChange} placeholder="Type name to sign..." required />;
    return <input type="text" id={id} className="form-input" value={value || ''} onChange={onChange} required />;
  };

  // ====== UI ======
  // Important: put toolbar in a dedicated container with marginTop + zIndex to avoid header overlap
  return (
    <div>
      ...
      {/* ===== Toolbar (fixes AI button alignment) ===== */}
      <div
        className="dg-toolbar"
        style={{
          display: 'flex',
          gap: '1rem',
          marginBottom: '1.5rem',
          flexWrap: 'wrap',
          // key fix: make toolbar sit below header and not overlap
          position: 'relative',
          zIndex: 1,
          marginTop: '0.25rem'
        }}
      ...
      {quickAIMode && (
        <div className="card" style={{ marginBottom: '2rem' }}>
          {aiStep === 1 && (
            <>
              <h2 style={{ fontSize: '1.25rem', fontWeight: 600, marginBottom: '1rem' }}>⚡ Quick AI Generate - Step 1</h2>
          ...
          {aiStep === 2 && (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                <h2 style={{ fontSize: '1.25rem', fontWeight: 600 }}>⚡ Step 2: Fill Placeholders for "{aiDocType}"</h2>
              ...
              <form onSubmit={handleAISaveDocument}>
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  <input type="text" value={aiDocumentName} onChange={(e) => setAiDocumentName(e.target.value)} className="form-input" required />
                </div>
                  ...
                  <>
                    ...
                    <div className="placeholder-grid" style={{ maxHeight: '400px', overflowY: 'auto', marginBottom: '1.5rem' }}>
                      {aiPlaceholders.map((placeholder) => (
                        <div key={placeholder} className="form-group-inline" style={{ marginBottom: '0.75rem', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                          <label style={{ minWidth: 160 }}>{placeholder}</label>
                          <div style={{ flex: 1 }}>{renderPlaceholderInput(placeholder, true)}</div>
                        </div>
                      ))}
                    </div>
                  </>
            ...
            {placeholders.length > 0 && (
              <div className="placeholder-grid" style={{ maxHeight: '400px', overflowY: 'auto', marginBottom: '1.5rem' }}>
                {placeholders.map((placeholder) => (
                  <div key={placeholder} className="form-group-inline" style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.75rem' }}>
                    <label style={{ minWidth: 160 }}>{placeholder}</label>
                    <div style={{ flex: 1 }}>{renderPlaceholderInput(placeholder, false)}</div>
                  </div>
                ))}
              </div>
            )}
        ...
        {loadingDocuments ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {documents.map((doc) => {
              ...
              return (
                <div key={doc.id} className="card document-list-item-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                  ...
                  <div className="document-actions" style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <div style={{
                      ...
                      <select
                        style={{ padding: '4px 8px', borderRadius: '6px', border: 'none', fontSize: '0.875rem' }}
                        onChange={(e) => {
                      ...
                      {translating && currentDocIdForTranslate === doc.id && <div className="spinner-small" style={{ width: '14px', height: '14px' }} />}
                    </div>
                  </div>
                </div>
              );
      ...
      {editingDocument && (
        <DocumentEditor document={editingDocument} onClose={() => setEditingDocument(null)} onSave={handleEditorSave} />
      )}
    </div>
  );
}
