...
const handlePreviewTemplate = async (id) => {
  try {
    ...
    setPreviewTemplate(res?.data?.data || null);
    setShowPreview(true);
  } catch (err) {
...
<div style={{display: 'flex', gap: '0.5rem'}}>
    ...
    <button
      ...
      className="btn btn-danger btn-delete-template"
    >
  ...
  <div style={previewStyles.overlay}>
    <div style={previewStyles.sheet}>
      ...
      <p style={previewStyles.subTitle}>
        📄 {manualTemplateType || "No document type"}
      </p>

      {manualDescription && (
        <p style={previewStyles.description}>{manualDescription}</p>
      )}

      <hr style={{ margin: "1rem 0" }} />

      <h3 style={previewStyles.sectionTitle}>
        Clauses Preview ({selectedClauses.length})
      </h3>

      {selectedClauses.length === 0 ? (
        <p style={{ color: "#64748b" }}>No clauses added.</p>
      ) : (
        selectedClauses.map((clause, index) => (
          <div key={index} style={previewStyles.clauseCard}>
            <h4 style={previewStyles.clauseTitle}>
              {index + 1}. {clause.clause_type}
              <span style={previewStyles.badge}>{clause.category}</span>
            </h4>
            <p style={previewStyles.clauseContent}>{clause.content}</p>
          </div>
        ))
      )}
    </div>
